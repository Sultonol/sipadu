                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            @if(auth()->user()->role === 'user')
                                <a href="{{ route('user.complaints') }}" class="text-sm font-medium text-gray-700 hover:text-blue-600">Pengaduan Saya</a>
                            @else
                                <a href="{{ route('admin.complaints.list') }}" class="text-sm font-medium text-gray-700 hover:text-blue-600">Kelola Pengaduan</a>
                            @endif
                        </div>
                    </li>
                    ...
                    <div class="p-6">
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <p class="text-gray-800 leading-relaxed whitespace-pre-wrap">{{ $complaint->description ?? 'Tidak ada deskripsi tersedia.' }}</p>
                                </div>
                            </div>
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                            ...
                                            @foreach($images as $image)
                                                <div class="relative group cursor-pointer" @click="showImageModal = true; currentImage = '{{ asset('storage/' . $image) }}'">
                    ...
                    <div class="p-6">
                        <div class="flow-root">
                            <ul class="-mb-auto">
                                @php
                                    $statusHistory = [
                                        ['status' => 'pending', 'date' => $complaint->created_at, 'description' => 'Pengaduan diterima dan menunggu review'],
                                    ];
                                    
                                    if($complaint->status !== 'pending') {
                                        $statusHistory[] = ['status' => $complaint->status, 'date' => $complaint->updated_at, 'description' => 'Status pengaduan diperbarui'];
                                    }
                                @endphp
                                    ...
                                    <li>
                                        <div class="relative {{ $index < count($statusHistory) - 1 ? 'pb-8' : '' }}">
                                            ...
                                            <div class="relative flex space-x-3">
                                                <div class="w-8 h-8 bg-{{ $statusInfo['color'] }}-100 rounded-full flex items-center justify-center">
            ...
            <div class="space-y-6">
                            ...
                            <div class="flex items-start">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-{{ $status['color'] }}-700">
                                        @if($complaint->status === 'pending')
                                            Pengaduan Anda sedang menunggu review dari tim admin. Mohon bersabar, kami akan segera memproses pengaduan Anda.
                                        @elseif($complaint->status === 'accepted')
                                            Selamat! Pengaduan Anda telah diterima dan diverifikasi oleh tim admin. Pengaduan akan segera diproses oleh instansi terkait.
                                        @elseif($complaint->status === 'in_progress')
                                            Pengaduan Anda sedang dalam proses penanganan oleh instansi terkait. Tim sedang bekerja untuk menyelesaikan masalah yang Anda laporkan.
                                        @elseif($complaint->status === 'completed')
                                            Pengaduan Anda telah diselesaikan! Terima kasih atas partisipasi Anda dalam membangun masyarakat yang lebih baik.
                                        @elseif($complaint->status === 'rejected')
                                            Pengaduan Anda ditolak. Silakan hubungi admin untuk informasi lebih lanjut atau ajukan pengaduan baru dengan informasi yang lebih lengkap.
                                        @else
                ...
                @if(auth()->user()->role === 'user')
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                        ...
                        <div class="p-6">
                            <div class="space-y-3">
                                <a href="{{ route('user.complaints') }}" class="w-full flex items-center justify-between p-3 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors group">
                                    <div class="flex items-center">
                                ...
                                <a href="{{ route('panduan') }}" class="w-full flex items-center justify-between p-3 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors group">
                                    <div class="flex items-center">
